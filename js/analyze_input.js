...
//In that file, write a program that receives user input and analyzes the input differently if the input is a number
// or a string. If the input is a string, it will analyze the string differently if it is length 1.
...
//Design your program to include a switch and at least one if/else. You must also make separate functions for each of
// the following pieces of program functionality:
...
function analyzeInput(x) {
    if ((x === Number) && (x % 2 === 0)) {
        return "Your Number is Edd"
    } else if ((x === Number) && (x % 2 === 1)) {
